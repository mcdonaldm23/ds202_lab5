...
```{r}
ggplot(diabetes_clean,aes(y=hip,x=waist,alpha=0.5)) + geom_point(shape=1, size=.5) +geom_smooth()+ facet_wrap(~frame) + labs(title='Weight by Frame',x='Waist', y='Hip')
```
...
```{r}
#Inference is what you are given a data set and have to get an output from the date.
