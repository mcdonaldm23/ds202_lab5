...
```{r}
# Using the log base along the x-axis. The down side to this is that the means are all the same.
```
...
```{r}
ggplot(diabetes_clean,aes(glyhb_star, ratio))+ geom_point(size=.5)+ geom_smooth()
```

```{r}
ggplot(diabetes_clean,aes(glyhb_star, bp.1s))+ geom_point(size=.5)+ geom_smooth()
```

```{r}
ggplot(diabetes_clean,aes(glyhb_star, age))+ geom_point(size=.5)+ geom_smooth()
```

```{r}
ggplot(diabetes_clean,aes(gender,glyhb_star))+ geom_boxplot()
```

```{r}
ggplot(diabetes_clean,aes(glyhb_star, hip))+ geom_point(size=.5)+ geom_smooth()
```

```{r}
ggplot(diabetes_clean,aes(glyhb_star, weight))+ geom_point(size=.5)+ geom_smooth()
```
...
```{r}
ggplot(diabetes_clean,aes(y=hip,x=waist,alpha=0.5)) + geom_point(shape=1, size=.1)+ facet_wrap(~frame) + labs(title='Waist vs. Hip by Frame',x='Waist (in.)', y='Hip (in.)')
```
...
```{r}
#You cannot constantly go back and forth from spread and gather because as you gather the information it must all become one. If you spead it, it forgets about the original one, creating a completely new column.
```
...
```{r}
#Based on these results, we can conclude that these variables are not significant because of how small the P-value is. We would choose to reject the null hypothesis based on the F-statistic. To improve this model, more variables could be added. 
#Based on question 12, we think a linear model would be the right approach. If you look back at that question, you can see a positive linear relationship exists.
```
